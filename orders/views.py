...
def detail(request, order_id):
    ...
    get_results=order.get_total_price()
    form = CouponForm()
    return render(request, 'orders/order.html',  {'order': order, 'form': form ,'get_results':get_results} )
...
def payment(request, order_id, price):
    ...
    result = client.service.PaymentRequest(
        MERCHANT, amount, description, request, mobile, CallbackURL)
...
def verify(request):
    if request.GET.get('Status') == 'OK':
        ...
        messages.error(request, 'Transaction was not successful', 'danger')
        return redirect('shop:home')
